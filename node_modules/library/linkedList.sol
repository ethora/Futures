...
library DoublyLinkedList {
    ...
    function find(data storage self, bytes32 _value) public returns (uint80) {
    ...
    // Iterator interface
    function iterate_start(data storage self) public returns (uint80) { return self.first; }
    function iterate_valid(data storage self, uint80 _index) public returns (bool) { return _index - 1 < self.items.length; }
    function iterate_prev(data storage self, uint80 _index) public returns (uint80) { return self.items[_index - 1].prev; }
    function iterate_next(data storage self, uint80 _index) public returns (uint80) { return self.items[_index - 1].next; }
    function iterate_get(data storage self, uint80 _index) public returns (bytes32) { return self.items[_index - 1].data; }
}
